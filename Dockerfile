...
WORKDIR /src
COPY ["LibreriaWeb.csproj", "."]
RUN dotnet restore "./LibreriaWeb.csproj"
COPY . .
WORKDIR "/src/."
RUN dotnet build "./LibreriaWeb.csproj" -c $BUILD_CONFIGURATION -o /app/build

# Questa fase viene usata per pubblicare il progetto di servizio da copiare nella fase finale
FROM build AS publish
ARG BUILD_CONFIGURATION=Release
RUN dotnet publish "./LibreriaWeb.csproj" -c $BUILD_CONFIGURATION -o /app/publish /p:UseAppHost=false
...
ENTRYPOINT ["dotnet", "LibreriaWeb.dll"]